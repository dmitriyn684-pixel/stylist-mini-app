...
async function verify(viewport, filename) {
  ...
  const checks = {
    ...
    visualBrandbookLinks: await page.locator('a[href$="dimkoff-brandbook-2026-visual-v2.pdf"]').count(),
    legacyVisualBrandbookLinks: await page.locator('a[href$="dimkoff-brandbook-2026-visual.pdf"]').count(),
  ...
  if (checks.legacyVisualBrandbookLinks < 1) throw new Error('Expected a legacy Visual Brandbook link');
  ...
  const pdfs = [
    ...
    'dimkoff-brandbook-2026-visual-v2.pdf',
  ];
